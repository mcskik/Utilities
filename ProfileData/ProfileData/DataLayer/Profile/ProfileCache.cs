...
using System.Data;
using System.Text;
using ProfileData.DataLayer.Variables;
...
namespace ProfileData.DataLayer.Profile
{
    ...
    public class ProfileCache
    {
        ...
        public bool FetchBool(string name)
        {
            bool boolean = false;
            string text = Fetch(name);
            bool.TryParse(text, out boolean);
            return boolean;
        }

        public DateTime FetchDateTime(string name)
        {
        ...
        public string Dump()
        {
            ...
            for (int stackLevel = 0; stackLevel < _variableStack.Count; stackLevel++)
            {
                //dump.Append(String.Format(@"Stack Level: {0}{1}", stackLevel, Environment.NewLine));
                foreach (var variable in _variableStack[_stackLevel])
                {
                    dump.Append(String.Format(@"{0} = ""{1}""{2}", variable.Key, variable.Value.Display(), Environment.NewLine));
                }
            }
