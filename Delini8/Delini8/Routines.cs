...
namespace Delini8
{
	...
	public class Routines
	{
		...
		public static string FileServer(string psFileSpec) {
			int nPos;
			string sFileServer;
			psFileSpec = psFileSpec.Trim();
			if ( psFileSpec.Length < 3 ) {
				sFileServer = string.Empty;
			} else if ( psFileSpec.Substring(0,2) == "\\\\" ) {
				nPos = psFileSpec.IndexOf("\\",2);
				if ( nPos > -1 ) {
		...
		public static string FileDrive(string psFileSpec) {
			string sFileDrive;
			psFileSpec = psFileSpec.Trim();
		...
		public static string AbsoluteSpec(string psCurrentSpec, string psRelativeSpec) 
		{
			...
			//Determine the type of slashes used in the file specs.
			if ( psCurrentSpec.IndexOf("\\") > -1 && psCurrentSpec.IndexOf("/") == -1 && psRelativeSpec.IndexOf("/") == -1 ) {
				sCdMk = "\\";
				sCdUp = "..\\";
			} else if ( psCurrentSpec.IndexOf("/") > -1 && psCurrentSpec.IndexOf("\\") == -1 && psRelativeSpec.IndexOf("\\") == -1 ) {
				sCdMk = "/";
				sCdUp = "../";
			} else {
				//Error, no recognised slashes found in file Specs.
				sSpec = string.Empty;
				return sSpec;
			}
			...
			if ( nPos > -1 ) {
				...
				if ( FileServer(psRelativeSpec) == string.Empty && FileDrive(psRelativeSpec) == string.Empty ) {
					sSpec = psCurrentSpec + sCdMk + psRelativeSpec;
				} else {
					sSpec = psRelativeSpec;
				}
			}
			return sSpec;
		}
