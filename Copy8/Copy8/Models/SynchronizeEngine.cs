...
namespace Copy8
{
    ...
    public class SynchronizeEngine
    {
        ...
        /// </remarks>
        public void Process(Comparisons<Comparison> comparisons, DateTime startAllDateTime)
        {
            Process(comparisons, startAllDateTime, true);
        }
        ...
        /// </remarks>
        public void Process(Comparisons<Comparison> comparisons, DateTime startAllDateTime, bool allowDelete)
        {
            sameCount = 0;
            deleteCount = 0;
            removeCount = 0;
        ...
        private void CreateDirectory(string targetDirectory)
        {
            ...
            if (!Directory.Exists(targetDirectoryInfo.FullName))
            {
                if (Interrupt.Reason != "Cancel")
                {
                    try
                    {
                        Administrator.Tracer.WriteTimedMsg("I", String.Format(@"Creating  : ""{0}""", targetDirectoryInfo.FullName));
                        Directory.CreateDirectory(targetDirectoryInfo.FullName);
                    }
                    catch (Exception ex)
                    {
                        Administrator.Tracer.WriteTimedMsg("E", String.Format(@"Error while creating : ""{0}""", targetDirectoryInfo.FullName));
        ...
        private void RemoveDirectory(string targetDirectory)
        {
            ...
            if (Directory.Exists(targetDirectoryInfo.FullName))
            {
                if (Interrupt.Reason != "Cancel")
                {
                    try
                    {
                        Administrator.Tracer.WriteTimedMsg("I", String.Format(@"Removing    : ""{0}""", targetDirectoryInfo.FullName));
                        targetDirectoryInfo.Delete();
                    }
                    catch (Exception ex)
                    {
                        Administrator.Tracer.WriteTimedMsg("E", String.Format(@"Error while removing : ""{0}""", targetDirectoryInfo.FullName));
                        Administrator.Tracer.WriteTimedMsg("E", ex.Message);
                    }
                }
            }
        ...
        private void CopyFileConditional(Comparison comparison)
        {
            ...
            string targetDirectory = Path.GetDirectoryName(targetFileSpec);
        ...
        private void CopyFileConditional(string sourceFileSpec, string targetFileSpec, string sourceFileName, string targetDirectory)
        {
            ...
            try
            {
                ...
                if (targetFileInfo.Exists)
                {
                    ...
                    {
                        if (CompareDateTimeStamps(sourceFileInfo.LastWriteTime, targetFileInfo.LastWriteTime) > 0)
                        {
                            doCopy = true;
                        }
                        else
                ...
                if (doCopy)
                {
                    try
                    {
                        ...
                        sourceFileInfo.CopyTo(Path.Combine(targetDirectory, sourceFileName), true);
        ...
        private void DeleteFile(string targetFileSpec)
        {
            ...
            if (targetFileInfo.Exists)
            {
                long fileLength = targetFileInfo.Length;
                if (Interrupt.Reason != "Cancel")
                {
                    try
                    {
                        ...
                        Administrator.Tracer.WriteTimedMsg("I", String.Format(@"{0} - Deleted", targetFileInfo.Name));
                    }
                    catch (Exception ex)
                    {
                        Administrator.Tracer.WriteTimedMsg("E", String.Format(@"Error while deleting : ""{0}""", targetFileInfo.Name));
                        Administrator.Tracer.WriteTimedMsg("E", ex.Message);
                    }
                }
            }
