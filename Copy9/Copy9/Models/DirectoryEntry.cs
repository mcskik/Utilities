...
namespace Copy9
{
    ...
    public class DirectoryEntries : List<DirectoryEntry>
    {
        ...
        public void Load(string fileSpec, InterpretationEnum interpretation, Connector connector)
        {
            ImportedRowCount = 0;
            this.Clear();
                ...
                if (sr.Peek() > -1)
                {
                    ...
                    do
                    {
                        ...
                        if (values.Count > 1)
                        {
                            ...
                            {
                                if (connector.HlqPrefix == Connector.EXCLAMATION_MARK || connector.HlqPrefix == Connector.PERCENT_SIGN)
                                {
                                    entry.TargetHlq = connector.StripDrivePrefix(entry.TargetHlq);
                                    entry.TargetHlq = connector.HlqPrefix + @":\" + entry.TargetHlq;
                                }
                                entry.StdHlq = entry.TargetHlq;
                            }
                            if (connector.HlqPrefix == Connector.EXCLAMATION_MARK || connector.HlqPrefix == Connector.PERCENT_SIGN)
                            {
                                string hlq = entry.StdHlq;
                                hlq = connector.HlqPrefix + hlq.Substring(1);
                                entry.StdHlq = hlq;
                            }
                            this.Add(entry);
                        }
        ...
        {
            ProfileData.DataLayer.Profile.FileHelper.PathCheck(fileSpec);
    ...
    public class DirectoryEntry
    {
        ...
        public DirectoryEntry(Google.Apis.Drive.v3.Data.File file)
        {
            StdHlq = string.Empty;
            StdDir = string.Empty;
            StdFile = string.Empty;
            StdSize = file.Size ?? 0;
            StdDate = file.ModifiedTime ?? DateTime.MinValue;
            SourceDate = StdDate;
            TargetDate = StdDate;
            SourceHlq = string.Empty;
            SourceFile = string.Empty;
            TargetHlq = string.Empty;
            TargetFile = string.Empty;
            StdType = file.FileExtension ?? "dir";
            CtlComparison = COMPARISON_EMPTY;
            EntryId = file.Id;
            EntryName = file.Name;
            EntryPath = string.Empty;
